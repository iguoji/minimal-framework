...
class Pool
{
     ...
     */
    public function __construct(int $size, array $configs, string $token, callable $constructor = null)
    {
        // 计算每台机器的平均连接数
    ...
    public function get($key = null)
    {
        ...
        // 得到连接
        $conn = $this->channels[$key]->pop($timeout);
        Coroutine::getContext()[$this->token()] = $conn;
        if (false === $conn) {
     ...
     * 归还连接
     */
    public function put($key, $conn) : void
    {
        // 将连接存放到通道
    ...
    private function make($key)
    {
        ...
        try {
            ...
            if (!is_null($this->constructor)) {
                ...
                $conn = $constructor($config);
            } else if (!is_null($config['handle'])) {
                if (is_callable($config['handle'])) {
                    // 使用配置里的构造器
                    $conn = $config['handle']($config);
                } else {
                    // 使用配置里的构造类
                    $conn = new $config['handle']($config);
                }
            } else {
